...
// attempt two - correct passing tests
// see video of two sum: https://www.youtube.com/watch?v=Ivyh3V4QolA
function twoSum(nums: number[], target: number): number[] {
  // important to use hash map
  let previousNumbers = {};

  for (let index = 0; index < nums.length; index++) {
    let currentNumber = nums[index];
    let neededValue = target - currentNumber;
    let index2 = previousNumbers[neededValue];

    if (index2 != null) {
      return [index2, index];
    } else {
      previousNumbers[currentNumber] = index;
    }
  }
}
